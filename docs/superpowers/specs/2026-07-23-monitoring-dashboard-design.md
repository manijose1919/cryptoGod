...
Provide a clean, professional, bug-tested, read-only monitoring GUI for the CRYPTO_GOD trading bot, accessible over a real HTTPS URL, hosted on the VPS (`31.97.7.138`, `/opt/trading-bot`).
...
- Reachable at `https://31-97-7-138.sslip.io` behind a trusted Let's Encrypt cert and an HTTP basic-auth password.
...
Browser ──HTTPS──▶ nginx (TLS + basic-auth) ──proxy──▶ localhost:3033 (Node)
        https://31-97-7-138.sslip.io                    ├─ GET /monitor            → monitor.html (static)
...
- Issue Let's Encrypt cert for `31-97-7-138.sslip.io` (sslip.io resolves the hostname to `31.97.7.138`; HTTP-01 challenge on port 80).
...
- **Live verification:**
  ...
  - Load `https://31-97-7-138.sslip.io` → browser shows padlock (trusted cert) and prompts for the password.
