...
- **Time-gate leak (Check 4)**: 3 trades entered during nominally-blocked hours 0–1 UTC (GROVE_USD ×2, TAOUSD ×1), all well after the timegate feature (`afeecc4`, 2026-06-30) and the baseline reset. The gate is not fully blocking hour 0 and hour 1 entries for at least these two tickers/strategies. Root cause not investigated (out of scope for this analysis-only task) — recommend a follow-up bug investigation, since a leaking time gate undermines the edge basis for that feature.
  - **RESOLVED 2026-07-15 (investigation):** not a gate malfunction — a scope gap. The 3 trades were SNIPER_CRYPTOCOM ×2 and MEAN_REVERSION ×1; those entry paths (plus BREAKOUT) never call `checkTimeGate`, which was wired only into TREND/MOMENTUM/SCALP per the documented 2026-06-30 scope ("blocked for TREND+MOMENTUM entries", edge measured on 382 TREND+MOMENTUM trades). Zero TREND/MOMENTUM entries have occurred during blocked hours since the deploy (3 apparent TREND hits on 2026-06-30 predate the 19:29 UTC deploy). Gate coverage was extended to the remaining strategies in a follow-up commit (see CHANGELOG 2026-07-15).
