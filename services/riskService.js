/**
 * Risk Metrics & Kelly Criterion Service (JS Version)
 *
 * Provides:
 ...
 */
export function updateEquityCurve(portfolioValue) {
  const now = Date.now();
 ...
 */
export function resetEquityTracking(initialValue) {
...
export function calculateRiskMetrics(
  ...
  initialBudget,
  ruinThreshold = 50  // Stop at 50% drawdown
) {
  const now = Date.now();
 ...
 */
function calculateRiskOfRuin(winRate, payoffRatio, ruinThreshold) {
 ...
 */
export function calculateKellyCriterion(trades, minTrades = 20) {
  const completedTrades = trades.filter(t => t.type === 'SELL' && t.pnl !== undefined);
...
export function runMonteCarloSimulation(
  ...
  numTrades = 100,
  numSimulations = 1000,
  ruinThreshold = 0.5  // 50% drawdown = ruin
) {
  const completedTrades = trades.filter(t => t.type === 'SELL' && t.pnl !== undefined);
