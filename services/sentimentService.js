/**
 * Enhanced Sentiment Analysis Service (JS Version)
 *
 ...
 */
export function calculateSentimentFromMarketData(candles, asset) {
  ...
  const recentVolume = veryRecent.reduce((sum, c) => sum + c.v, 0) / veryRecent.length;
  ...
  const bullishCandles = recent.filter(c => c.c > c.o).length;
 ...
 */
function getDefaultSentiment(asset) {
  return {
    asset,
    polarity: 'NEUTRAL',
    score: 0,
 ...
 */
export function detectSentimentBurst(currentSentiment, correlatedAssets = []) {
 ...
 */
export function detectSentimentRegime(sentimentSignals, priceChange24h = 0) {
 ...
 */
export function applySentimentFilter(sentiment, tradeDirection, minSentimentScore = -30) {
 ...
 */
export function calculateSentimentConfidenceAdjustment(sentiment, strategy) {
  let adjustment = 0;
  const reasons = [];
 ...
 */
export function getCorrelatedMemeAssets(asset) {
  const memeGroups = {
    // Dog coins
