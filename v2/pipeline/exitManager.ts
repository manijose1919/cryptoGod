...
export async function checkExits(
  ...
  for (const trade of openTrades) {
    ...
    // Trailing activation is floored at a fee multiple (see config) so the trail
    // can never arm on a move too small to clear round-trip fees — strategies may
    // ask to trail earlier, but economics set the minimum. This is the only
    // read-site of trailActivatePercent, so the floor applies engine-wide.
    // Note: the BE-stop trigger below (0.6 × activation) inherits the floor.
